...
test('boxes the card with every row the same width', () => {
  const rows = renderPairingCard(TOKEN, createPaint('truecolor'), 120).map(stripVTControlCharacters);
...
test('drops the box on a terminal too narrow for it', () => {
  const rows = renderPairingCard(TOKEN, createPaint('none'), 40);
  assert.ok(rows.every((row) => !/[╭╮╰╯│]/.test(row)));
  assert.ok(rows.some((row) => row.includes(TOKEN)));
  assert.ok(rows.some((row) => row.includes('npx nudge-mode extension')));
});
